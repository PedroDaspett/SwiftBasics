...
/*
 ...
 // For example, if an app has 2 screens and user is moving from screen 1 to screen 2. We only want to allocate (create) screen 2 WHEN we need it (ie. (so that would be) when the user clicks a button to segue to screen 2). When we get to screen 2, we may want to deallocate (destroy) screen 1.
 ...
 // That is why every single view, every single screen in SwiftUI is a struct. It is ecause structs are faster, have lower memory footprint and this is why SwiftUI apps are highly performant
